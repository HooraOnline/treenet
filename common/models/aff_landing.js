...
/*    const userId=data.userId;
    ...
    let entity={url:data.url,device:'mobile',type:'landing',cdate:new Date()};
  ...
  Model.addClick =  (data, callback)=>{
        const userId=data.userId;
        if(!userId){
          callback(new Error('An error occurred'));
          return
        }
        let entity={clickerId:userId,memberId:data.memberId,url:data.url,type:'rising',cdate:new Date()};
        return Model.updateOrCreate(entity, function(err, res) {
          if(err){
            callback(null,{errorCode:17, lbError:err, errorKey:'server_public_error',errorMessage:'خطا در اضافه کردن شماره. دوباره سعی کنید.'});
          }else{
  ...
  Model.getUserLandings = function (params, callback) {
